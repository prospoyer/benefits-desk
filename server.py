#!/usr/bin/env python3
"""Benefits Desk demo server. Standard library only.

  python3 server.py          # http://127.0.0.1:8400 locally
  PORT=8080 python3 server.py  # hosting platforms set PORT and it binds 0.0.0.0

Serves the Benefits Desk at /benefits/ (/ redirects there) with no-cache headers, and a small JSON API:

  GET  /api/health              {"ok": true}
  GET  /api/state/benefits      this visitor's saved demo progress, or {}
  PUT  /api/state/benefits      save this visitor's progress (JSON body, 64 KB max)
  POST /api/reset/benefits      clear this visitor's progress
...
MAX_AGE = 60 * 60 * 24 * 30
LOCK = threading.Lock()
ALLOWED = re.compile(r"^/(benefits|shared)(/|$)")
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path in ("/", "/index.html"):
            self.send_response(302); self.send_header("Location", "/benefits/"); self.end_headers(); return
        if path == "/api/health":
...
if __name__ == "__main__":
    ...
    print(f"Benefits Desk on http://{host}:{port}/benefits/", flush=True)
